...
// API alternativa para UniTV (faz autenticação internamente)
...
// Live TV streams (COM autenticação via sessão UniTV)
router.get('/live/:username/:password/:streamId.:extension', XtreamController.authenticateUniTVSession, async (req, res) => {
...
router.get('/live/:username/:password/:streamId', XtreamController.authenticateUniTVSession, async (req, res) => {
...
// Movie streams (COM autenticação via sessão UniTV)
router.get('/movie/:username/:password/:streamId.:extension', XtreamController.authenticateUniTVSession, async (req, res) => {
    const { streamId, extension } = req.params;
...
router.get('/movie/:username/:password/:streamId', XtreamController.authenticateUniTVSession, async (req, res) => {
    const { streamId } = req.params;
    const stream = xtreamController.vodStreams.get(streamId);
...
// Series streams (COM autenticação via sessão UniTV)
router.get('/series/:username/:password/:seriesId/:seasonId/:episodeId.:extension', XtreamController.authenticateUniTVSession, async (req, res) => {
...
router.get('/series/:username/:password/:episodeId.:extension', XtreamController.authenticateUniTVSession, async (req, res) => {
